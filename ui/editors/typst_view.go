...
type TypstEditor struct {
	*view.BaseView
	...
	pendingProject bus.ProjectCreatedEvent
}
...
func (te *TypstEditor) update(gtx C) {
	...
	filters := settingsmodel.ShortcutFilters(shortcutConfig, nil, settingsmodel.ShortcutTogglePreview)
	for {
		...
		switch event := e.(type) {
		case key.Event:
			if settingsmodel.ShortcutMatches(shortcutConfig, settingsmodel.ShortcutTogglePreview, event) {
				te.togglePreview(gtx)
...
func (te *TypstEditor) Layout(gtx layout.Context, th *theme.Theme) layout.Dimensions {
	...
	if te.pendingProject.Path != "" {
		...
		te.pendingProject = bus.ProjectCreatedEvent{}
		te.srv.EventBus().Emit(bus.TopicProjectCreate, event)
		te.srv.RefreshWindow()
	}
	return dims
}
...
func (te *TypstEditor) onPaste(text string) string {
	...
	studentMode := te.srv.Settings().FileInterface().Mode == settingsmodel.FileInterfaceModeStudent
	root, inProject := assetProjectRoot(
		te.srv.CurrentProjectDir(),
		te.targetFile,
		studentMode,
	)
	if inProject {
	...
	te.pendingProject = bus.ProjectCreatedEvent{
		Path:            project,
		SwitchWorkspace: !studentMode,
		OpenFile:        document,
		ReplacedFile:    te.targetFile,
	}
	te.targetFile = document
	return ""
}
